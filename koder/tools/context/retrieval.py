"""Context retrieval tool using embeddings for enhanced code understanding."""
...
class ContextRetrievalInput(BaseModel):
    ...
    query: str = Field(description="Query or task description to find relevant context for")
    max_results: int = Field(default=3, description="Maximum number of context results to retrieve")
    file_filter: Optional[str] = Field(default=None, description="Optional file path filter (e.g., '*.py' or 'src/**')")


@registry.register("code", needs_approval=False, is_read_only=True)
class ContextRetrievalTool(ReadOnlyTool):
    """Tool for retrieving relevant code context using embeddings."""
    ...
    description: str = (
        ...
        "Provides semantic understanding of code relationships beyond simple text matching."
    )
    ...
    def _ensure_initialized(self):
        ...
        try:
            ...
            # Auto-index if no data exists
            try:
                # Check if any documents exist by attempting a search
                test_results = self._retriever.vector_store.similarity_search("test", k=1)
            ...
            # If embedding initialization fails, create a mock retriever
            self._retriever = None
            self._initialized = True
    ...
    def _auto_index_workspace(self):
        ...
        try:
            ...
            skip_dirs = {
                ...
                ".nuxt", "site-packages", "vendor", "cache", "logs"
            }

            # Define file priorities for intelligent indexing
            file_priorities = {
                # High priority - core project files
                "setup.py": 10, "pyproject.toml": 10, "package.json": 10,
                "requirements.txt": 10, "Cargo.toml": 10, "go.mod": 10,
                "main.py": 10, "app.py": 10, "index.js": 10, "main.go": 10,

                # High priority - configuration files
                "README.md": 9, "CHANGELOG.md": 8, "CONTRIBUTING.md": 7,
                ".env.example": 8, "Dockerfile": 9, "docker-compose.yml": 9,

                # Medium priority - source files
                ".py": 7, ".js": 7, ".ts": 7, ".go": 7, ".rs": 7, ".java": 7,
                ".jsx": 6, ".tsx": 6, ".vue": 6, ".svelte": 6,

                # Medium priority - documentation
                ".md": 5, ".rst": 5, ".txt": 4,

                # Lower priority - config and data
                ".json": 5, ".yaml": 5, ".yml": 5, ".toml": 5, ".ini": 4,
                ".sh": 5, ".sql": 5, ".dockerfile": 6,

                # Test files (important but lower priority)
                "test_": 4, "_test.py": 4, ".test.js": 4, "spec.": 4,
            }
            ...
            print(f"🔍 Smart indexing: Found {len(files_to_index)} eligible files, indexing top {max_files}")
            ...
            for file_path, priority in files_to_index[:max_files]:
                try:
                    ...
                    success = self._retriever.index_file(str(file_path), chunk_size=1500)
                    if success:
                        indexed_count += 1
            ...
            if indexed_count > 0:
                print(f"✅ Successfully indexed {indexed_count} files for semantic search")

                # Log what was indexed
                indexed_files = [str(f[0]) for f in files_to_index[:indexed_count]]
                high_priority_count = sum(1 for _, p in files_to_index[:indexed_count] if p >= 7)
                print(f"   • {high_priority_count} high-priority files (setup, main, config)")
                print(f"   • {indexed_count - high_priority_count} other source files")
            else:
                print("⚠️  No files were indexed. Check file permissions and workspace structure.")

        except Exception as e:
            print(f"❌ Auto-indexing failed: {str(e)}")
            # Don't raise - allow tool to continue without context

    def _run(self, query: str, max_results: int = 3, file_filter: Optional[str] = None) -> str:
        """Execute the context retrieval."""
        self._ensure_initialized()

        if not self._retriever:
            return "Context retrieval is not available. Embedding system initialization failed."

        try:
            # Get relevant context
            context = self._retriever.get_relevant_context(query, k=max_results)

            # Add file filtering if specified
            if file_filter and context != "No relevant context found.":
                try:
                    filtered_docs = self._retriever.search_code(query, k=max_results, file_filter=file_filter)
                    if filtered_docs:
    ...
    async def _arun(self, query: str, max_results: int = 3, file_filter: Optional[str] = None) -> str:
